...
NUM_OF_ON_BOARD_DI = 16
DESCRIPTION = "On board digital input"

class MK201DiChannelEditor(wx.Panel):
...
class MK201DiEditor (MK201IOEditor):
    ...
    def __init__(self, parent, window, controler):
        ...
        self.inputs = []
        for i in range(0, NUM_OF_ON_BOARD_DI):
            channel = MK201DiChannelEditor(self, i)
    ...
    def SetData(self, config):
        ...
        """ Если устанавливаемых параметров меньше чем надо, то уставливаются значения
         по умолчанию """
        if len(config) < NUM_OF_ON_BOARD_DI:
            config = self.DefaultConfig[:]
            self.RefreshModel()
